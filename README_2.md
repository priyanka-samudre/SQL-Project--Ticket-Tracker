...
Built as a portfolio piece for a SQL-focused Technical Services role — the domain deliberately mirrors that day-to-day work: investigating customer data issues, writing reports and dashboards, and keeping SQL correct and performant.
...
sql-ticket-tracker/
├── sql/
│   ├── 01_schema.sql              -- database, 4 tables, keys, constraints, indexes
│   ├── 02_import_data.sql         -- loads the CSVs via BULK INSERT
│   ├── 03_views.sql               -- 2 reporting views
│   ├── 04_stored_procedures.sql   -- 3 stored procedures
│   ├── 05_report_queries.sql      -- 6 analytical report queries
│   └── 06_performance_basics.sql  -- 3 before/after optimisation examples
├── data/
│   ├── customers.csv  (15 rows)
│   ├── agents.csv     (6 rows)
│   ├── categories.csv (5 rows)
│   └── tickets.csv    (3,000 rows)
├── docs/                          -- plain-English, line-by-line explanation of every SQL file
└── README.md
```

An Excel dashboard (`SupportTicketReport.xlsx`) built from this dataset is summarised at the end of this README.

---

## How to run it

1. SQL Server 2019+ (Developer Edition is free) with SSMS, or Azure SQL.
2. Run `sql/01_schema.sql` to create the database and tables.
3. Open `sql/02_import_data.sql`, set `@DataPath` to your local `data` folder, and run it. (No-code alternative: SSMS → right-click database → Tasks → Import Flat File.)
4. Run `sql/03_views.sql`, then `sql/04_stored_procedures.sql`.
5. Explore `sql/05_report_queries.sql` and `sql/06_performance_basics.sql`.
...
Key design points: CHECK constraints reject invalid tiers/priorities/statuses and impossible dates (resolved before created); foreign keys prevent orphan tickets; and foreign-key columns are explicitly indexed (SQL Server doesn't do this automatically).
...
**4. Each customer's latest ticket** (CTE + `ROW_NUMBER`) — returns one row per customer: their most recent ticket.
...
## Performance examples (`06_performance_basics.sql`)

Three before/after pairs that return identical results but perform very differently. Run with `SET STATISTICS IO ON` and *Include Actual Execution Plan* to compare:

1. **Non-SARGable predicate** — `WHERE YEAR(CreatedAt)=2026` (scan) vs a date range (seek).
2. **Row-by-row vs set-based** — a `WHILE` loop of per-customer counts vs one `GROUP BY`.
3. **`SELECT *` vs named columns** — fetching everything vs letting a covering index answer the query.
